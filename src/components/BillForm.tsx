...
import { formatDateVn, today } from "@/lib/period";
import { categoryIcon } from "@/lib/avatars";
import SplitFlow from "./SplitFlow";
import Avatar from "./Avatar";
import { IconAlert, IconBack, IconCheck, IconSpinner, ICON_SIZE } from "./Icons";
    ...
    <div className="step-indicator" aria-hidden="true">
      {STEPS.map((s, i) => (
        <div key={s.n} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? "1 1 auto" : "0 0 auto" }}>
          ...
          >
            <span className="step-indicator-dot">{s.n}</span>
            <span className="step-indicator-label">{s.label}</span>
          </div>
          {i < STEPS.length - 1 && (
            <span className={`step-indicator-line ${step > s.n ? "done" : ""}`} />
          )}
        </div>
      ))}
    </div>
...
export default function BillForm({
  ...
  const splitValid = participants.length > 0 && !exactMismatch;
  const canSave = infoValid && splitValid;
  ...
  function toggleParticipant(userId: number) {
    setDraft({
      ...draft,
      selected: { ...draft.selected, [userId]: !draft.selected[userId] },
    });
  }
  ...
  const payer = members.find((m) => m.userId === draft.paidBy);
    ...
    <div className="stack">
      ...
      {step === 2 && (
        <div className="card card-pad stack">
          ...
          {/* Sơ đồ dòng tiền trực quan: bấm avatar để bật/tắt tham gia */}
          <SplitFlow
            payerName={payer?.name ?? "?"}
            payerAvatar={payer?.avatar}
            total={total}
            onTogglePerson={toggleParticipant}
            people={members.map((m) => ({
              userId: m.userId,
              name: m.name,
              avatar: m.avatar,
              amount: preview.get(m.userId) ?? 0,
              selected: Boolean(draft.selected[m.userId]),
            }))}
          />

          {draft.splitMode !== "equal" && participants.length > 0 && (
            <div>
              {participants.map((m) => (
                <div className="split-row" key={m.userId} style={{ gridTemplateColumns: "1fr auto" }}>
                  <span>
                    {m.name}
        ...
        <div className="card card-pad stack">
          <div className="row">
            <span className="cat-ic" aria-hidden="true">
              ...
              <img src={categoryIcon(draft.category)} alt="" />
            </span>
            <span className="entry-main">
              <span className="entry-title" style={{ fontSize: 17, fontWeight: 600 }}>
                {draft.title}
              </span>
              <span className="faint" style={{ display: "block" }}>
                {formatDateVn(draft.spentOn)} · {categoryLabel(draft.category)}
              </span>
            </span>
          </div>

          <p className="num amount-lg" style={{ margin: "4px 0" }}>
            {formatVnd(total)}
          </p>

          <div className="row" style={{ gap: 6 }}>
            <span className="tag">{SPLIT_MODE_LABEL[draft.splitMode]}</span>
            <span className="tag">{payer?.name ?? "?"} ứng tiền</span>
          </div>

          {draft.note && <p className="muted" style={{ margin: 0 }}>{draft.note}</p>}
          ...
          <div>
            {participants.map((m) => (
              <div className="split-row" key={m.userId} style={{ gridTemplateColumns: "auto 1fr auto" }}>
                <Avatar avatarId={m.avatar} name={m.name} size={28} />
                <span>
                  {m.name}
                  {m.userId === currentUserId && (
                    <span className="tag tag-blue" style={{ marginLeft: 6 }}>
                ...
                <span className="num split-amount">{formatVnd(preview.get(m.userId) ?? 0)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="row">
        ...
        {step < 3 ? (
          <button
            ...
            onClick={() => setStep((step + 1) as Step)}
            disabled={(step === 1 && !infoValid) || (step === 2 && !splitValid)}
          >
            Tiếp theo
          </button>
        ) : (
